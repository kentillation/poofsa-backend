...
class CashierAuthController extends Controller
{
    ...
    public function cashierLogin(Request $request)
    {
        $validated = $request->validate([
            'cashier_email' => ['required', 'email'],
            'cashier_password' => ['required', 'string'],
        ]);

        try {
            Log::info('Cashier login attempt', [
                'email' => $validated['cashier_email'],
                'ip' => $request->ip(),
            ]);
            ...
            // Find user
            $user = CashierModel::where('cashier_email', $validated['cashier_email'])->first();
            ...
            // Generate token
            $token = $user->createToken('auth_token', ['cashier:access'], $this->getTokenExpiration($remember))->plainTextToken;
            ...
            Log::error('Login exception', [
                ...
                'ip' => $request->ip(),
                'email' => $validated['cashier_email'] ?? 'unknown',
            ]);
    ...
    protected function checkLoginAttempts(Request $request)
    {
        if (RateLimiter::tooManyAttempts($this->throttleKey($request), $this->maxAttempts)) {
            ...
            Log::warning('Account locked due to too many attempts', [
                'email' => $request->input('cashier_email'),
                'ip' => $request->ip(),
