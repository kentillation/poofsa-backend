...
class KitchenAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $validated = $request->validate([
                'kitchen_email' => ['required', 'string'],
                'kitchen_password' => ['required', 'string'],
                'device_name' => ['nullable', 'string', 'max:255'],
            ]);

            $remember = $request->boolean('remember');

            $user = \App\Models\KitchenModel::where('kitchen_email', $validated['kitchen_email'])->first();
    
            if (!$user || !Hash::check($validated['kitchen_password'], $user->kitchen_password)) {
                RateLimiter::hit($this->throttleKey($request));
                throw ValidationException::withMessages([
                    'kitchen_email' => [trans('auth.failed')],
                ]);
            }
    ...
    protected function checkLoginAttempts(Request $request)
    {
        if (RateLimiter::tooManyAttempts($this->throttleKey($request), $this->maxAttempts)) {
            ...
            throw ValidationException::withMessages([
                'kitchen_email' => [trans('auth.throttle', [
                    'seconds' => $seconds,
    ...
    {
        return Str::transliterate(Str::lower($request->input('kitchen_email')) . '|' . $request->ip());
    }
