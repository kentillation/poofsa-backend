...
class KitchenAuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            $validated = $request->validate([
                'kitchen_personnel_email' => ['required', 'string'],
                'kitchen_personnel_password' => ['required', 'string'],
            ]);

            $credentials = [
                'kitchen_personnel_email' => $validated['kitchen_personnel_email'],
                'password' => $validated['kitchen_personnel_password']
            ];

            $remember = $request->boolean('remember');

            if (!Auth::guard('kitchen_personnel')->attempt($credentials, $remember)) {
                RateLimiter::hit($this->throttleKey($request));
                throw ValidationException::withMessages([
                    'kitchen_personnel_email' => [trans('auth.failed')],
                ]);
            }

            /** @var \App\Models\Kitchen\KitchenPersonnelModel $user */
            $user = Auth::guard('kitchen_personnel')->user();
            ...
            return response()->json([
                ...
                'user_id' => $user->kitchen_personnel_id,
                'branch_id' => $user->branch_id,
                'branch_name' => $branch ? $branch->branch_name : null,
                'branch_address' => $branch ? $branch->branch_address : null,
                'branch_contact_number' => $branch ? $branch->branch_contact_number : null,
            ])->withCookie($cookie);
        } catch (\Exception $e) {
            Log::error('Kitchen login error: ' . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function logout(Request $request)
    {
        try {
            ...
            Auth::guard('kitchen_personnel')->logout();
            $request->session()->invalidate();
            $request->session()->regenerateToken();
            return response()->json(['message' => 'Logged out successfully.']);
        } catch (\Exception $e) {
            Log::error('Kitchen logout error: ' . $e->getMessage());
            return response()->json(['message' => 'Logged out successfully.']);
        }
    }
    ...
    protected function checkLoginAttempts(Request $request)
    {
        if (RateLimiter::tooManyAttempts($this->throttleKey($request), $this->maxAttempts)) {
            ...
            throw ValidationException::withMessages([
                'kitchen_personnel_email' => [trans('auth.throttle', [
                    'seconds' => $seconds,
    ...
    {
        return Str::transliterate(Str::lower($request->input('kitchen_personnel_email')) . '|' . $request->ip());
    }
