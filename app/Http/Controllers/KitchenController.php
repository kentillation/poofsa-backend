...
class KitchenController extends Controller
{
    public function getCurrentOrders()
    {
        try {
            ...
            $data = TransactionModel::select(
                ...
                ->where('tbl_transaction_orders.station_id', 2)
    ...
    public function getKitchenProductDetails($transactionId)
    {
        try {
            if (!$transactionId) {
                return response()->json([
            ...
            $transaction = TransactionModel::where('transaction_id', $transactionId)
                ...
                ->with(['orders' => function ($query) {
                    $query->where('station_id', 2); // Assuming 2 is the station ID for Kitchen
                }])
    ...
    public function updateKitchenProductStatus(Request $request)
    {
        try {
            $input = $request->all();
            ...
            // If status is updated to 2 (completed), update the stock
            if ($nextStatus == 2) {
                ...
                // $transaction->save();
                // event(new RealTimeMessage($transaction->order_status_id));
                // event(new OrderStatusUpdated($transaction->order_status_id));
                // END: for real-time event
            }
            $transaction = TransactionModel::where('transaction_id', $input['transactionId'])->first();
            if (!$transaction) {
                return response()->json([
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    // 'order_status_id' => $transaction->order_status_id,
                    // 'previous_status_id' => $currentStatus
                ]
            ], 200);
        } catch (\Exception $e) {
    ...
    //             ], 422);
    //         }
    //         $transaction = TransactionOrdersModel::where('transaction_id', $input['transactionId'])->first();
    //         if (!$transaction) {
    //             return response()->json([
    //                 'status' => false,
    //                 'message' => 'Transaction not found'
    //             ], 404);
    //         }
    //         $currentStatus = $transaction->station_status_id;
    //         $nextStatus = $currentStatus % 2 + 1;
    //         $transaction->station_status_id = $nextStatus;
    //         $transaction->save();

    //         // event(new OrderStatusUpdated($transaction->station_status_id));
    //         return response()->json([
    //             'status' => true,
    //             'message' => 'Kitchen product status updated successfully',
    //             'data' => [
    //                 'transaction_id' => $transaction->transaction_id,
    //                 'station_status_id' => $transaction->station_status_id,
    //                 // 'previous_status_id' => $currentStatus
    //             ]
    //         ], 200);
    //     } catch (\Exception $e) {
    //         Log::error('Kitchen product status update failed: ' . $e->getMessage());
    //         return response()->json([
    //             'status' => false,
    //             'message' => 'Failed to update kitchen product status',
    //             'error' => config('app.debug') ? $e->getMessage() : null
    //         ], 500);
    //     }
    // }
