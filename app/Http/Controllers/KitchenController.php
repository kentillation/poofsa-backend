...
class KitchenController extends Controller
{
            ...
            $data = OrdersModel::select(
                ...
                ->map(function ($rows) {
                    ...
                    return [
                        ...
                        'order_status_id' => $first->order_status_id,
    ...
    public function getKitchenProductDetails($orderId)
    {
        try {
            if (!$orderId) {
                return response()->json([
            ...
            $orders = OrdersModel::where('order_id', $orderId)
                ...
                // ->with(['orders' => function ($query) { $query->where('station_id', 2);}]) // if order is for Kitchen only
            ...
            $formattedOrders = $orders->orders
                // ->filter(function ($order) { return $order->station_id == 2; }) // if order is for Kitchen only
                ->map(function ($order) {
                    return [
                        'order_id' => (int)$order->order_id ?? 'N/A',
                        'station_id' => (int)$order->station_id ?? 'N/A',
                        'product_id' => (int)$order->product->product_id ?? 'N/A',
                        ...
                        'station_status_id' => (int)$order->station_status_id,
                    ];
                });
            ...
            return response()->json([
                ...
                'data' => [
                    ...
                    'table_number' => (int)$orders->table_number,
                    ...
                    'total_quantity' => (int)$orders->total_quantity,
    ...
    private function deductStockForOrder($orderId, $stationId = 2)
    {
    ...
    public function updateOrderStatus(Request $request)
    {
        try {
            ...
            if ($orders->order_status_id == 1) {
                // Deduct stock and update Kitchen station status
                $this->deductStockForOrder($orders->order_id, 2);
                ...
                if ($trnsctn_orders) {
                    ...
                    event(new OrderStatusUpdated('Kitchen: Order in table ' . $tableNumber . ' is ready.', $stationId));
                }
            } else {
                return response()->json([
                    'status' => false,
